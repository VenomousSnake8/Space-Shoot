// IMPORTANT: Shoutout to dwmkerr (https://github.com/dwmkerr) for the original code. I made a handful of corrections to fit my design better, but the VAST majority of it is written by him
    ...
    for(var rank = 0; rank < ranks; rank++){
        for(var file = 0; file < files; file++) {
            invaders.push(new Invader(
                (game.width / 2) + ((files/2 - file) * 475 / files),
                (game.gameBounds.top + rank * 33),
                rank, file, 'Invader'));
        }
    }
...
PlayState.prototype.draw = function(game, dt, ctx) {
    ...
    for(var i=0; i<this.bombs.length; i++) {
        var bomb = this.bombs[i];
        ctx.fillRect(bomb.x - 2, bomb.y - 2, 10, 10);
    }
